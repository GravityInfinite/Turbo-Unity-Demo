...
namespace GravitySDK.PC.Main
{
    ...
    public class GravitySDKInstance
    {
        ...
        private GravitySDKTask mTask {
            get {
                return GravitySDKTask.SingleTask();
            }
            set {
                this.mTask = value;
            }
        ...
        public GravitySDKInstance(string appId, string server, string instanceName, GravitySDKConfig config, MonoBehaviour mono = null)
        {
            this.mMono = mono;
            sMono = mono;
        ...
        public virtual string DistinctId()
        {
            ...
            this.mDistinctID = (string)GravitySDKFile.GetData(this.mAppid,GravitySDKConstant.DISTINCT_ID, typeof(string));
        ...
        {
            this.mAccountID = (string)GravitySDKFile.GetData(this.mAppid,GravitySDKConstant.ACCOUNT_ID, typeof(string));
        ...
        public virtual void Flush()
        {
            if (mEventSaveOnly == false) {
                ...
                ResponseHandle responseHandle = delegate (Dictionary<string, object> result) {
                    int eventCount = 0;
                    if (result != null && !(result.ContainsKey("is_response_error") && Convert.ToBoolean(result["is_response_error"])))
                    {
                        int flushCount = 0;
                        ...
                        if (!string.IsNullOrEmpty(this.mConfig.InstanceName()))
                        {
                            eventCount = GravitySDKFileJson.DeleteBatchTrackingData(flushCount, this.mConfig.InstanceName());
                        }
                        else
                        {
            ...
            if (mEventSaveOnly == false) {
                ...
                ResponseHandle responseHandle = delegate (Dictionary<string, object> result) {
                    ...
                    if (result != null && !(result.ContainsKey("is_response_error") && Convert.ToBoolean(result["is_response_error"])))
                    {
                        if (!string.IsNullOrEmpty(this.mConfig.InstanceName()))
                        {
                            eventCount = GravitySDKFileJson.DeleteBatchTrackingData(batchSize, this.mConfig.InstanceName());
                        }
                        else
                        {
        ...
        public virtual void SetSuperProperties(Dictionary<string, object> superProperties)
        {
            ...
            string propertiesStr = (string)GravitySDKFile.GetData(this.mAppid, GravitySDKConstant.SUPER_PROPERTY, typeof(string));
            ...
            GravitySDKFile.SaveData(this.mAppid, GravitySDKConstant.SUPER_PROPERTY, GravitySDKJSON.Serialize(this.mSupperProperties));
        }
