...
static ufixed64_t ntp_localtime_get_ufixed64() {
    ...
    return ge_ufixed64((uint32_t)tv.tv_sec + kSecondsFrom1900To1970, tv.tv_usec * (pow(2, 32) / USEC_PER_SEC));
}

+ (GENTPServer *)defaultServer {
    static GENTPServer *server = nil;
    static dispatch_once_t onceToken = 0;
    dispatch_once(&onceToken, ^{
        server = [[GENTPServer alloc] init];
    });
    return server;
}
...
- (BOOL)syncWithError:(NSError *__autoreleasing _Nullable *_Nullable)error {
    @synchronized (self) {
        ...
        packet = ge_hton_ntp_packet(packet);
        ...
        packet = ge_ntoh_ntp_packet(packet);
        const double T[4] = {
            ge_ufixed64_as_double(packet.originate_timestamp),
            ge_ufixed64_as_double(packet.receive_timestamp),
            ge_ufixed64_as_double(packet.transmit_timestamp),
            ge_ufixed64_as_double(ntp_localtime_get_ufixed64()),
        };
        _offset = ((T[1] - T[0]) + (T[2] - T[3])) / 2.0;
        
        GELogDebug(@"%s %f", __func__, _offset);
        return YES;
    }
}
