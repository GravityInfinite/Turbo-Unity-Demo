...
#define kTDDyldPropertyNames @[@"GEPerformance"]
...
#if TARGET_OS_IOS
static CTTelephonyNetworkInfo *__ge_TelephonyNetworkInfo;
#endif

@interface GEDeviceInfo ()
...
@implementation GEDeviceInfo

+ (void)load {
#if TARGET_OS_IOS
    __ge_TelephonyNetworkInfo = [[CTTelephonyNetworkInfo alloc] init];
#endif
}
...
+ (GEDeviceInfo *)sharedManager {
    ...
    static GEDeviceInfo *manager;
    dispatch_once(&onceToken, ^{
        manager = [[GEDeviceInfo alloc] init];
    });
    return manager;
}
...
- (instancetype)init {
    ...
    if (self) {
        self.libName = @"iOS";
        self.libVersion = GEPublicConfig.version;
        ...
        _automaticData = [self ge_collectProperties];
    }
    return self;
}
...
-(NSDictionary *)getAutomaticData {
    ...
    [dic addEntriesFromDictionary:[GEDeviceInfo getAPMParams]];
    _automaticData = dic;
...
- (NSDictionary *)ge_collectProperties {
    ...
    if (![GEPresetProperties disableDeviceId]) {
        [p setValue:_deviceId forKey:@"$device_id"];
    }
    
#if TARGET_OS_IOS
    if (![GEPresetProperties disableCarrier]) {
        CTCarrier *carrier = nil;
        NSString *carrierName = @"";
    #ifdef __IPHONE_12_0
            if (@available(iOS 12.1, *)) {
                NSArray *carrierKeysArray = [__ge_TelephonyNetworkInfo.serviceSubscriberCellularProviders.allKeys sortedArrayUsingSelector:@selector(compare:)];
                carrier = __ge_TelephonyNetworkInfo.serviceSubscriberCellularProviders[carrierKeysArray.firstObject];
                if (!carrier.mobileNetworkCode) {
                    carrier = __ge_TelephonyNetworkInfo.serviceSubscriberCellularProviders[carrierKeysArray.lastObject];
                }
            }
    #endif
        
        if (!carrier) {
            carrier = [__ge_TelephonyNetworkInfo subscriberCellularProvider];
        }
    ...
    if (![GEPresetProperties disableLibVersion]) {
        [p setValue:self.libVersion forKey:@"$lib_version"];
    }
    if (![GEPresetProperties disableManufacturer]) {
        [p setValue:@"Apple" forKey:@"$manufacturer"];
    }
   

#if TARGET_OS_IOS
    if (![GEPresetProperties disableDeviceModel]) {
        [p setValue:[self ge_iphoneType] forKey:@"$model"];
    }
    
    if (![GEPresetProperties disableLib]) {
        [p setValue:self.libName forKey:@"$lib"];
    }
    
    if (![GEPresetProperties disableOs]) {
        [p setValue:@"iOS" forKey:@"$os"];
    }
    if (![GEPresetProperties disableOsVersion]) {
        UIDevice *device = [UIDevice currentDevice];
        [p setValue:[device systemVersion] forKey:@"$os_version"];
    }
    if (![GEPresetProperties disableScreenWidth]) {
        CGSize size = [UIScreen mainScreen].bounds.size;
        [p setValue:@((NSInteger)size.width) forKey:@"$screen_width"];
    }
    if (![GEPresetProperties disableScreenHeight]) {
        CGSize size = [UIScreen mainScreen].bounds.size;
        [p setValue:@((NSInteger)size.height) forKey:@"$screen_height"];
    }
    
    if (![GEPresetProperties disableDeviceType]) {
...
#endif
    if (![GEPresetProperties disableSystemLanguage]) {
    ...
    [p addEntriesFromDictionary:[GEDeviceInfo getAPMParams]];
...
- (NSDictionary *)getDeviceUniqueId {
    ...
    GEKeychainHelper *wrapper = [[GEKeychainHelper alloc] init];
    NSString *deviceIdKeychain = [wrapper readDeviceId];
    NSString *installTimesKeychain = [wrapper readInstallTimes];
    
    BOOL isExistFirstRecord = [[[NSUserDefaults standardUserDefaults] objectForKey:@"gravity_isfirst"] boolValue];
    if (!isExistFirstRecord) {
        _isFirstOpen = YES;
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"gravity_isfirst"];
    ...
    GEFile *file = [[GEFile alloc] initWithAppid:[GravityEngineSDK sharedInstance].appid];
    if (deviceIdKeychain.length == 0 || installTimesKeychain.length == 0) {
...
+ (NSString *)currentRadio {
    ...
    @try {
        ...
        if (@available(iOS 12.0, *)) {
            NSDictionary *serviceCurrentRadio = [__ge_TelephonyNetworkInfo serviceCurrentRadioAccessTechnology];
...
#endif
        if (currentRadio == nil && [__ge_TelephonyNetworkInfo.currentRadioAccessTechnology isKindOfClass:[NSString class]]) {
            currentRadio = __ge_TelephonyNetworkInfo.currentRadioAccessTechnology;
        }
