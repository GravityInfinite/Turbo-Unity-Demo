...
static void ge_swizzledMethod_2(id self, SEL _cmd) {
    Method aMethod = class_getInstanceMethod([self class], _cmd);
    GESwizzle *swizzle = (GESwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
    if (swizzle) {
...
static void ge_swizzledMethod_3(id self, SEL _cmd, id arg) {
    Method aMethod = class_getInstanceMethod([self class], _cmd);
    GESwizzle *swizzle = (GESwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
    if (swizzle) {
...
static void ge_swizzledMethod_4(id self, SEL _cmd, id arg, id arg2) {
    Method aMethod = class_getInstanceMethod([self class], _cmd);
    GESwizzle *swizzle = (GESwizzle *)[swizzles objectForKey:(__bridge id)((void *)aMethod)];
    if (swizzle) {
...
static void (*ge_swizzledMethods[MAX_ARGS - MIN_ARGS + 1])() = {ge_swizzledMethod_2, ge_swizzledMethod_3, ge_swizzledMethod_4};
#pragma clang diagnostic pop

@implementation GESwizzler
...
+ (GESwizzle *)swizzleForMethod:(Method)aMethod {
    return (GESwizzle *)[swizzles objectForKey:MAPTABLE_ID(aMethod)];
}
...
+ (void)setSwizzle:(GESwizzle *)swizzle forMethod:(Method)aMethod {
...
+ (void)swizzleSelector:(SEL)aSelector
    ...
    if (!aMethod) {
        GELogInfo(@"SwizzleException:Cannot find method for %@ on %@",NSStringFromSelector(aSelector), NSStringFromClass(aClass));
        return;
    }
    
    uint numArgs = method_getNumberOfArguments(aMethod);
    if (numArgs < MIN_ARGS || numArgs > MAX_ARGS) {
        GELogError(@"Cannot swizzle method with %d args", numArgs);
    }
    
    IMP swizzledMethod = (IMP)ge_swizzledMethods[numArgs - 2];
    [GESwizzler swizzleSelector:aSelector onClass:aClass withBlock:aBlock andSwizzleMethod:swizzledMethod named:aName];
}
...
+ (void)swizzleSelector:(SEL)aSelector
    ...
    Method aMethod = class_getInstanceMethod(aClass, aSelector);
    if (!aMethod) {
         GELogError(@"Cannot find method for %@ on %@", NSStringFromSelector(aSelector), NSStringFromClass(aClass));
    }
    
    BOOL isLocal = [self isLocallyDefinedMethod:aMethod onClass:aClass];
    GESwizzle *swizzle = [self swizzleForMethod:aMethod];
    ...
    if (isLocal) {
        if (!swizzle) {
            ...
            @try {
                swizzle = [[GESwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
        ...
        if (!class_addMethod(aClass, aSelector, aSwizzleMethod, method_getTypeEncoding(aMethod))) {
             GELogError(@"Could not add swizzled for %@::%@, even though it didn't already exist locally", NSStringFromClass(aClass), NSStringFromSelector(aSelector));
        }
        ...
        if (aMethod == newMethod) {
             GELogError(@"Newly added method for %@::%@ was the same as the old method", NSStringFromClass(aClass), NSStringFromSelector(aSelector));
        }
        
        GESwizzle *newSwizzle = [[GESwizzle alloc] initWithBlock:aBlock named:aName forClass:aClass selector:aSelector originalMethod:originalMethod];
